...
# GaussianNB pipeline
# -----------------------------
# GaussianNB itself has almost no hyperparameters.
# We'll tune var_smoothing (stabilizes estimates) using GroupKFold.
gnb = GaussianNB()

pipe = Pipeline([
    ("imputer", SimpleImputer(strategy="median")),
    ("scaler", StandardScaler()),     # helps because GaussianNB assumes Gaussian features
    ("clf", gnb),
])

param_grid = {
    "clf__var_smoothing": np.logspace(-12, -6, 7)  # small search, fast
}
...
#   Aggregate window probs -> subject mean_prob vs apnea_label (AHI>=5)
# -----------------------------
...
out_win.to_csv("dataset_csv/gnb_test_window_predictions.csv", index=False)

subject_scores.to_csv("dataset_csv/gnb_test_subject_scores.csv", index=False)

print("\nSaved:")
print("- dataset_csv/gnb_test_window_predictions.csv")
print("- dataset_csv/gnb_test_subject_scores.csv")
end_time = time.time()
